...
- **Production agentic browsing** — the registry has a mock CDP provider for contract tests and a first native `playwright` provider backed by the upstream Beater browser crates; run-attached sessions, crash cleanup, credentials, and live browser e2e remain.
