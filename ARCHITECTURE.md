...
- **Production agentic browsing** — the registry has a mock CDP provider for contract tests and a native `playwright` provider backed by the upstream Beater browser crates; `scripts/playwright-browser-gate.cjs` proves a real Chromium tool call through the agent loop with run-scoped session identity, while persistent multi-call sessions, crash cleanup, and credentials remain.
