...
- **Production agentic browsing** — the registry has a mock CDP provider for contract tests and a native `playwright` provider backed by the upstream Beater browser crates; `scripts/playwright-browser-gate.cjs` proves real Chromium calls through the agent loop reuse one run-scoped session, and resume cleans stale browser runner markers before review/replay. Scoped credentials and authenticated browser e2e remain.
