...
The optional live provider smoke gate, `scripts/llm-live-provider-smoke.cjs`, reads the same environment keys and writes redacted logs plus `evidence.md` under `.beater/live-provider-smoke/`. It must not be run with keys pasted into command lines or committed files. The gate records provider name, model, base URL, run id, journal shape, and tool payload; it does not record request headers, raw key values, or provider error bodies.
