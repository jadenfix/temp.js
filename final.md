...
- `browserTool(..., {provider: "playwright"})` now reuses the pinned upstream Beater browser crates to launch Chromium through the Playwright runner, navigate to `input.url`, execute one optional browser action, and close the session after the tool call. Unit tests cover provider configuration, secret rejection, and action input shapes; live browser e2e remains.
...
| 9 | **Agentic browsing** — reuse beater-agents' CDP/Playwright crates as a tool provider | **partial:** `provider: "playwright"` reuses the upstream Beater browser crates for a real Chromium tool call and closes the session after execution; done still requires a live browser e2e where an agent completes a real task, with run-attached sessions and crash cleanup |
