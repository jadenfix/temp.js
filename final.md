...
- `browserTool(..., {provider: "playwright"})` now reuses the pinned upstream Beater browser crates to launch Chromium through the Playwright runner, navigate to `input.url`, execute one optional browser action, reuse the browser session across calls in one run, and close sessions when the agent run or synthetic MCP run reaches a terminal state. Unit tests cover provider configuration, secret rejection, run-scoped session reuse, action input shapes, and pre-driver origin rejection; `scripts/playwright-browser-gate.cjs` passed with two real Chromium tool calls through `beater agent run` and verified session reuse in SQLite.
...
| 9 | **Agentic browsing** — reuse beater-agents' CDP/Playwright crates as a tool provider | **partial:** `provider: "playwright"` reuses the upstream Beater browser crates, and `scripts/playwright-browser-gate.cjs` proves real Chromium tool calls through the agent loop with journal verification and run-scoped session reuse; done still requires crash/restart cleanup for browser sessions |
