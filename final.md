...
- Route modules can now export `agent.actions: [defineAction(...)]`; the hello contact action proves one route handler accepts a human form POST, appears in live `/mcp tools/list` with confirm/idempotency metadata, and executes through `/mcp tools/call` via the same journaled route handler path.
...
- The hello example now includes a route-scoped EventSource run stream panel that opens `/_beater/agent/runs/<run_id>/events` and renders `llm_partial` events in the browser; dev smoke tests prove the panel and transpiled client code ship in both the checked-in fixture and scaffolded app.
...
| 7 | **LLM streaming** — SSE to browser + partial-step journal records | **partial:** Anthropic SSE ingestion, partial-step journal records, a browser-consumable run events stream, and the hello EventSource run panel are implemented; done requires production-grade run history/navigation polish |
...
| 10 | **defineAction** — one definition → HTML form + MCP tool + OpenAPI + crawler metadata (§6b end state) | **partial:** route-bound `defineAction` now powers a human form POST and live MCP `tools/list` + journaled `tools/call`; `beater-connect` static `Action` definitions emit `forms.html`, OpenAPI, MCP catalog, and crawler metadata with matching semantics; done requires unifying runtime route actions with OpenAPI/crawler generation |
