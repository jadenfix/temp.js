...
| MCP workflow prompts | `initialize` advertises prompt capability, `prompts/list` exposes `beater.review_pr`, `beater.update_docs`, `beater.systems_design`, and `beater.choose_stack`, and `prompts/get` returns bounded text prompt messages with required-argument validation through the same authenticated `/mcp` endpoint |
