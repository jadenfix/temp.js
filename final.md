...
| Playwright browser provider | `scripts/playwright-browser-gate.cjs` installs the upstream runner dependencies in a temp dir, runs a local browser fixture plus Anthropic-compatible SSE mock, drives `beater agent run` through `provider: "playwright"`, and verifies a completed Chromium tool result in SQLite |
...
- `browserTool(..., {provider: "playwright"})` now reuses the pinned upstream Beater browser crates to launch Chromium through the Playwright runner, navigate to `input.url`, execute one optional browser action, and close the session after the tool call. Unit tests cover provider configuration, secret rejection, action input shapes, and pre-driver origin rejection; `scripts/playwright-browser-gate.cjs` passed with a real Chromium session through `beater agent run` and verified the completed tool result in SQLite.
...
| 9 | **Agentic browsing** — reuse beater-agents' CDP/Playwright crates as a tool provider | **partial:** `provider: "playwright"` reuses the upstream Beater browser crates, and `scripts/playwright-browser-gate.cjs` proves a real Chromium tool call through the agent loop with journal verification; done still requires run-attached sessions and crash cleanup |
