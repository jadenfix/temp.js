...
- `browserTool(..., {provider: "playwright"})` now reuses the pinned upstream Beater browser crates to launch Chromium through the Playwright runner, navigate to `input.url`, execute one optional browser action, and close the session after the tool call. Unit tests cover provider configuration, secret rejection, run-scoped session identity, action input shapes, and pre-driver origin rejection; `scripts/playwright-browser-gate.cjs` passed with a real Chromium session through `beater agent run` and verified the completed tool result plus run-scoped session id in SQLite.
...
| 9 | **Agentic browsing** — reuse beater-agents' CDP/Playwright crates as a tool provider | **partial:** `provider: "playwright"` reuses the upstream Beater browser crates, and `scripts/playwright-browser-gate.cjs` proves a real Chromium tool call through the agent loop with journal verification and run-scoped session identity; done still requires persistent multi-call sessions and crash cleanup |
