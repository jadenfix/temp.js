...
| 10 | **defineAction** — one definition → HTML form + MCP tool + OpenAPI + crawler metadata (§6b end state) | **partial:** `beater-connect` static `Action` definitions now emit `forms.html`, OpenAPI, MCP catalog, and crawler metadata with matching auth/confirm/idempotency semantics; done requires route-bound handlers that execute through both form posts and live `/mcp tools/list` + `tools/call` |
